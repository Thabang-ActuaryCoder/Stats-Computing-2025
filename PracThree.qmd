...
subtitle: "Part 3: Practical Day Five"
...
Carrier "OO" has only one observation hence the standard deviation is "NA", and also the rest are "0" because there is one unique distance between the values, so hence the standard deviation is 0.
...
flights |>
  summarise(
    total_flights = n(),
    delayed_departure_on_time_arrival = sum(dep_delay > 0 & 
                                              arr_delay <= 0, na.rm = TRUE),
    proportion = delayed_departure_on_time_arrival / total_flights
  )
...
# 1. Identify routes with multiple airlines
...
flights |>
  ...
  filter(n_airlines > 1) |>
  head() |>
  kable(caption = "Routes served by multiple airlines", digits = 0)

```

```{r}

# 2. Calculate average delays per airline on these routes
...
route_delays <- flights |>
  semi_join(multi_airline_routes, by = c("origin", "dest")) |>
...
route_delays |>
  arrange(origin, dest) |>
  head() |>
  kable(caption = "Average Arrival Delays per Airline on Shared Routes", digits = 2)

```

```{r}
# 3. Identify best/worst performing airlines per route

route_comparison <- route_delays |>
  group_by(origin, dest) |>
  summarise(
...
route_comparison |>
  ...
  head() |>
  kable(caption = "Route Performance Comparison: Best vs. Worst Airlines", digits = 2)

```

```{r}

# 4. Route with greatest difference

max_diff_route <- route_comparison |>
  filter(delay_diff == max(delay_diff, na.rm = TRUE))

max_diff_route |>
  kable(caption = "Route with the Largest Performance Gap", digits = 2)
...
a)  **Identify Typos:** in this dataset, the first 10 rows reveals that "disease_status" contains three unique entries (e.g `"healthy"`, `"diseased"`, and the typo `"Healthy"`) and also `"female"`, `"male"`, and the typo `"femal"`.

b\) **Detect Missing Data:** It’s easy to overlook missing values (NAs).

c\) **Verify Expected Values:** For instance, the `id` column should have 500 unique entries, and `view_cols` can help you confirm this.
